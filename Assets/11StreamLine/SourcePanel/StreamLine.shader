Shader "SourcePanel/SourceStreamLine"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            float4 _MainTex_ST;
			uniform float SourceStrength[256];
            ...
            {
			    float theta,psi = 0.0f;
			    for(int k = 0;k < 256;k++)
				{
				  if(SourceStrength[k] == 0.0f)break;
				  theta = atan2(i.uv.y - YC[k],i.uv.x - XC[k]);
				  //因为theta在180度和-180度时不连续
				  //所以将SourceStrength的精度控制在0.1以绘制连续的流线
                  SourceStrength[k] /= 10.0f;
				  SourceStrength[k] = floor(SourceStrength[k] * 10.0f)/10.0f;
				  psi += (SourceStrength[k] / (2 * 3.14159) * theta); 
				}
				psi += Vinf * i.uv.y;//均匀流造成的势能影响
				float4 col = float4(1.0f, 1.0f, 1.0f, 1.0f);
				//SourceStrength的精度为10/100 = 0.1，如果高于这个精度则流线不连续
