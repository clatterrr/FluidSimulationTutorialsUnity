...
public class VPManager : MonoBehaviour
{
	...
	//平板长度  平板的角度  平板的法向量  受到攻角影响的法向量
	float[] PanelLength, phi, delta, beta;

	float[,] I, J, Amatrix, AInverse;
	//切向加速度，压力参数，法向力因素(normal force coefficient)，轴向力因素(axial force coefficient)
	float[] Vt, Cp, CN, CA;
	...
	void Update()
	{
		...
		if (airfoiltype == AirfoilType.Cylinder)
		{
			for (int i = 0; i < PanelNumber; i++)
			{
				...
				float theta = -Mathf.PI * 2.0f / PanelNumber * i;//需要是顺时针
		...
		//应用Kutta条件，将第i行移到第i+1行，第一行为1000...0001
