import numpy as np
#Lax Wendroff for 1d Shallow Water Equation
...
for k in range(0,tmax-2):
    ...
    for i in range(0,nmax-1):
        U1pred[k,i] = 0.5*(U1[k,i+1]+U1[k,i] - c*(F1[k,i+1]-F1[k,i]))
        U2pred[k,i] = 0.5*(U2[k,i+1]+U2[k,i] - c*(F2[k,i+1]-F2[k,i]))
        h[k+1,i] = U1pred[k,i]
    ...
    for i in range(1,nmax-1):
        U1[k+1,i] = U1[k,i] - c*(F1[k+1,i] - F1[k+1,i-1])
        U2[k+1,i] = U2[k,i] - c*(F2[k+1,i] - F2[k+1,i-1])
        h[k+1,i] = U1[k+1,i]
