...
def RungeKutta2():
    ...
    return min(cfl * 1 / stepBound,1)


for t in range(0,100):
    oldData = data0.copy()
    TimeDelta = RungeKutta2()
    data0 = data0 + TimeDelta * ydot
    TimeDelta = RungeKutta2()
    data0 = (oldData + data0 + TimeDelta * ydot)/2
    for i in range(0,nmax):
        for j in range(0,nmax):
            oldData[i,j] = 0
            if abs(data0[i,j]) < 0.1:
                oldData[i,j] = 1
    plt.imshow(oldData, interpolation='none')
    test = 1